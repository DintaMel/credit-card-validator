...
app.post('/credit-card-validity', (req, res) => {
    // TODO: write tests?
    const body = req.body;
    if ('cc-number' in body == false) {
        return res.json({ isValid: false });
    }
    const creditCardNumber = req.body['cc-number'];
    if (Number.isNaN(Number(creditCardNumber))) {
        return res.json({ isValid: false });
    }
    const checkSumDigit = Number(creditCardNumber.slice(-1));
    const ccNumWithoutCheckSum = creditCardNumber.slice(0, -1);
